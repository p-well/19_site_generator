...
def render_index_page(page_template):
...
def create_article_page(config, site_basepath, rendered_page):
...
if __name__ == '__main__':
    ...
    rendered_index_page = render_index_page(index_page_template)
    create_index_page('site', rendered_index_page)
    for config in configs['articles']:
        article_filepath = 'articles/{}'.format(config['source'])
        ...
        rendered_article_page = render_page(
            article_page_template,
            config,
            html_obj
        )
        create_article_page(config, site_basepath, rendered_atricle_page)
